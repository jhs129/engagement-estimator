...
import { updateEpicSchema } from '@/types/story'
...
export async function PATCH(
  ...
  const result = await prisma.epic.updateMany({
    where: { id: eid, estimateId: id },
    data: parsed.data,
  })

  if (result.count === 0) return Response.json({ error: 'Not found' }, { status: 404 })

  const updated = await prisma.epic.findUnique({ where: { id: eid } })
...
export async function DELETE(
  ...
  await prisma.epic.deleteMany({ where: { id: eid, estimateId: id } })
